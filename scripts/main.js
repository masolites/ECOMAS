...
class UserSystem {
    ...
    static updateMiningRate(newRate) {
        TOKENOMICS.miningBaseRate = newRate;
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('socialLoginBtn').addEventListener('click', function(e) {
        e.preventDefault();
        document.getElementById('loginModal').style.display = 'flex';
    });
    ...
    document.getElementById('adminBtn').addEventListener('click', function() {
        const user = UserSystem.getCurrentUser();
        if (!user || user.email !== "admin@masolites.com") {
        ...
        users.forEach(user => {
            ...
            userEl.innerHTML = `
                ...
                <div>${user.cryptoWallet.balance.toFixed(6)} MZLx</div>
            `;
            usersList.appendChild(userEl);
        });
        
        document.getElementById('miningRate').value = TOKENOMICS.miningBaseRate;
        
        document.getElementById('adminModal').style.display = 'flex';
    });
    
    document.getElementById('saveSettings').addEventListener('click', function() {
        const newRate = parseFloat(document.getElementById('miningRate').value);
        if (newRate > 0) {
            UserSystem.updateMiningRate(newRate);
            alert('Mining rate updated successfully!');
        } else {
            alert('Invalid mining rate');
        }
    });
